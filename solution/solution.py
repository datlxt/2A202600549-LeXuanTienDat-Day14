...
class RAGASEvaluator:
    ...
    # -----------------------------------------------------------------------
    # BONUS — Custom metric: Conciseness
    # -----------------------------------------------------------------------
    def evaluate_conciseness(self, answer: str, expected: str) -> float:
        """Conciseness — penalize answers that are much longer than needed.

        StudyMate students want focused answers, not padding. We use the
        expert reference answer length as a target budget.

        Heuristic:
            a = #content tokens in answer, e = #content tokens in expected
            - answer no longer than the reference (a <= e) => 1.0 (concise)
            - longer answers degrade as the reference/answer ratio:  e / a
            Return 1.0 if either side is empty (nothing to penalize).

        Note: standalone metric — NOT part of overall_score() (which is fixed
        at the 3 core metrics the tests rely on).
        """
        answer_tokens = _tokenize(answer)
        expected_tokens = _tokenize(expected)
        if not answer_tokens or not expected_tokens:
            return 1.0
        if len(answer_tokens) <= len(expected_tokens):
            return 1.0
        return max(0.0, min(1.0, len(expected_tokens) / len(answer_tokens)))
    ...
    def evaluate_context_precision(
        ...
        # 1. Mark each chunk relevant / not relevant.
        ...
        # 2 + 3. Average Precision over the ranks where a relevant chunk appears.
        hits = 0
        ap_sum = 0.0
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "on a 0.0-1.0 scale and reply with a JSON object mapping each "
            "criterion to its score.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_lines}\n\n"
            'Respond ONLY with JSON, e.g. {"criterion": 0.8}.'
        )

        # 2. Call the judge LLM.
        raw = self.judge_llm_fn(prompt)

        # 3. Parse the response for scores; fall back to 0.5 per criterion.
        ...
        try:
            ...
            if isinstance(parsed, dict):
                ...
                # Keep any extra criteria the judge returned too.
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        # Flatten every numeric score across the batch.
        all_scores: list[float] = []
        per_entry_avg: list[float] = []
        for entry in scores_batch:
            entry_scores = list(entry.get("scores", {}).values())
            numeric = [float(s) for s in entry_scores]
            all_scores.extend(numeric)
            if numeric:
                per_entry_avg.append(sum(numeric) / len(numeric))
        ...
        # Positional bias: the first half of responses scores noticeably higher
        # than the second half (judge favouring whatever came first).
        positional_bias = False
        if len(per_entry_avg) >= 2:
            mid = len(per_entry_avg) // 2
            first_half = per_entry_avg[:mid]
            second_half = per_entry_avg[mid:]
            first_avg = sum(first_half) / len(first_half)
            second_avg = sum(second_half) / len(second_half)
            positional_bias = (first_avg - second_avg) > 0.2
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Preserve the original QAPair (keeps metadata/retrieved_contexts).
            result.qa_pair = pair
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for m in metrics:
            ...
            base_avg = _avg(baseline_results, m)
            out[f"new_avg_{m}"] = new_avg
            out[f"baseline_avg_{m}"] = base_avg
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # If more than one metric is tied at the lowest value, it's systemic.
        num_at_lowest = sum(1 for v in scores.values() if v == lowest)
        if num_at_lowest > 1:
    ...
    def generate_improvement_suggestions(
        ...
        # Targeted suggestions per failure type.
        playbook = {
            "hallucination": (
            ...
            "irrelevant": (
                "Improve prompt clarity and intent routing so answers address "
                "the actual question"
            ),
            "incomplete": (
            ...
            "off_topic": (
                "Add intent detection and topic guardrails to keep answers "
                "on subject"
            ),
            "refusal": (
                "Relax over-strict guardrails so the agent answers in-scope "
                "questions"
            ),
        }

        suggestions: list[str] = []
        # Prioritise by frequency (most common failure first).
        ...
        generic = [
            ...
            "Add automated regression tests as a CI/CD quality gate before deploy",
            "Calibrate the LLM-as-Judge rubric against human-labelled examples",
        ]
        for g in generic:
